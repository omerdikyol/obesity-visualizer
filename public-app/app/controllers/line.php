...
<script src="/obesity-visualizer/public-app/app/helper/chartFunctions.js"></script>
...
<script>
    ...
    function insertData(data) {
        ...
        // Add year to x-axis
        xAxis.domain([d3.min(data, function(d) {
                    return d.year;
                }),
        ...
        const yMax = d3.max(allValues) + 5;

        const roundedYMax = Math.round(10 * (yMax + 5)) / 10; // Round to nearest higher 10
        const roundedYMin = Math.round(10 * (yMin - 5)) / 10; // Round to nearest lower 10

        yAxis.domain([roundedYMin, roundedYMax])
            .range([height, 0]);
        ...
        var svgForZoom = d3.select(document.querySelector("svg"));

        var zoom = d3.zoom()
            .scaleExtent([1, 8]) // Set the minimum and maximum zoom levels
            .on("zoom", zoomed);

        svgForZoom.call(zoom); // Bind the zoom behavior to the SVG element

        function zoomed(event) {
            // Update the transform of the SVG element based on the zoom event
            svgForZoom.attr("transform", event.transform);
        }
